...
> Five-line orientation for any AI coding agent (Claude Code, Codex, Cursor, Cline, Aider, etc.) entering this repo. Generated by kernel `ingest-project.py` on 2026-05-20T11:15:24Z; canonical methodology lives in the cowork kernel.
...
Why: kernel canonical state (`_cowork/PROJECT-MAP.json`, `plugin/tools/`, `plugin/commands/`, the kernel-side `_cowork/dashboard-it-infra.json`) lives in the kernel folder. Every preset reads PROJECT-MAP.json + bare-name tool resolution from kernel during its Step-1 handshake. Single-mount Cowork tasks fail with `[BLOCK] kernel-mount-not-reachable` at handshake time.

See `../Cowork Project Prompt Enginner/_cowork/COWORK-SESSION-SETUP.md` for the canonical multi-mount setup recipe.

**Per-batch convention:** every non-trivial batch lands a run-log at the kernel `_cowork/runs/<UTC-stamp>-IM<n>.log` ending with a single `[END] outcome=...` line. The kernel-side `_cowork/dashboard-it-infra.json` is the WBS source of truth -- bump the relevant node's `last` and `status` via `tfi-edit.py --rewrite` (kernel-published bare-name tool).

**No-touch zones:** owner-defined; none at scaffold time. Add as patterns emerge.

*Generated by kernel KM316 ingest-project.py. Project prefix: IM. Mount: C:\CC\it-infra.*
